...
const { GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, REDIRECT_URI } = loadEnv;
...
router.get('/app/google/login',async (req,res)=>{ //if user accessing this api from apps rather than API //for app accessing
    const open = (await import("open")).default;
    open(AUTH_URL, { wait: false }); // Opens the URL without waiting for the browser to close
})

router.get('/site/google/login', (req, res) => { //if user is accessing this api from a website //for broser accessing
...
router.get('/callback', async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({msg:"Successfully authorised the client"})
    } catch (error) {
        return res.status(500).json({msg:"Error occured in getting authorization"})
    }
